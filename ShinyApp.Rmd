...
netflix <- netflix %>%
  mutate(
    ...
    decade = floor(release_year / 10) * 10  # Convert release year to decade
  ) %>%
...
ui <- fluidPage(
  ...
  titlePanel(div("🎬 Netflix Explorer: Discover Movies & Insights", style = "color: black; text-align: center;font-weight: bold;")),
...
server <- function(input, output, session) {
  ...
  output$recommendTable <- renderDT({
    datatable(
      filtered_data() %>%
        select(title, type, duration_mins, rating),  # Show only required columns
